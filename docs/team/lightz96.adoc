...
I take this module during Year 2 as an undergraduate while pursuing my Bachelor of Computer in Computer Science at NUS.
...
With THRIFT, users can keep track of their incoming and outgoing transactions by executing a single command (one of the requirements in this module)
and will not be worried about overspending. It is designed for NUS students who want to save money and prefer typing to other
inputs, like a mouse click. Below shows how THRIFT looks like:
...
convert your currency, clone transactions, filter transactions, and last but not least, undo and redo commands.
...
** Highlights: The implementation is challenging because I need to provide different sets of implementation for each undoable command
and ensure that each implementation works correctly.
...
** Justification: If users have forgotten about the filter they set for the list of transactions, they can refer to this filter text.
...
The following section demonstrates my documentation skills in providing technical details regarding the implementation of the undo and redo features.
