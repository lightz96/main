...
image::UndoAndRedo/initialUndoRedo.PNG[333, 300]
...
image::UndoAndRedo/addExpense.PNG[333, 300]
...
image::UndoAndRedo/updateExpense.PNG[346, 300]
...
Before returning the `UpdateCommand` object back to the callee, `UpdateCommand` is popped from `UndoStack` and pushes into `RedoStack`.

image::UndoAndRedo/undo.PNG[315, 300]
...
Before returning the `AddExpenseCommand` back to the callee, `AddExpenseCommand` is popped from `UndoStack` and pushes into `RedoStack`.

image::UndoAndRedo/undo2.PNG[315, 300]
...
image::UndoAndRedo/redo.PNG[310, 300]
...
The following activity diagram shows what happen if user executes command:

.Activity diagram of the command execution.
image::UndoAndRedo/RunCommandActivityDiagrams.png[401, 600]

==== Design considerations

===== Aspect: Data structure used to keep track of undoable command

* **Alternative 1 (current choice):** Use two stacks. One stack stores the command to undo, the other stack stores the command to redo.
** Pros: Easy to implement.
** Cons: Need to carefully manage the two stacks. For example, you need to pop command from one stack and push into another stack.
