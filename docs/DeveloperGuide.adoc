...
.. Test case: `untag i/1 t/LUNCH`, then `undo`. Next, use `clone i/1` to clone transaction, then redo the undone command using `redo` +
   Expected: No changes. Message shown in the status message indicating that there are no available commands for user to redo.
.. Test case: `list`, then redo the undone command using `redo` +
   Expected: No changes. Message shown in the status message indicating that there are no available commands for user to redo.
