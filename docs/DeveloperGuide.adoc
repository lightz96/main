...
*Undo* command enables reverting back to the previous state when user has made a mistake. *Redo* command does the opposite of *undo* command
when user realized that the command can be executed.
...
Every undoable commands will be stored in the `UndoStack` once it is executed. It will only be stored in `RedoStack` when user executed
*undo*.

==== Implementation
The below scenario illustrated how undo and redo feature works.

Step 1. Application is launched with empty transaction list. Both `UndoStack` and `RedoStack` are empty upon initializing `**PastUndoableCommands**`.

image::UndoAndRedo/initialUndoRedo.PNG[align="center", 333, 300]

Step 2. The user executes *add_expense n/Laksa v/4* to add laksa to the transaction list in THRIFT.
...
image::UndoAndRedo/addExpense.PNG[align="center", 333, 300]

Step 3. The user executes *update i/1 v/3.5* to update the cost of the laksa to $3.50.
...
image::UndoAndRedo/updateExpense.PNG[align="center", 346, 300]

Step 4. The user executes *undo* to revert the cost of the laksa back to $4.00.
The `UndoCommand` invokes the method `Model#getPreviousUndoableCommand()` to retrieve the `UpdateCommand` object.
Before returning the `UpdateCommand` back to the callee, `UpdateCommand` was pop from `UndoStack` and push into `RedoStack`.

image::UndoAndRedo/undo.PNG[align="center", 315, 300]

Step 5. The user executes *undo* to remove laksa from THRIFT's transaction list.
The `UndoCommand` invokes the method `Model#getPreviousUndoableCommand()` to retrieve the `AddExpenseCommand` object.
Before returning the `AddExpenseCommand` back to the callee, `AddExpenseCommand` was pop from `UndoStack` and push into `RedoStack`.

image::UndoAndRedo/undo2.PNG[align="center", 315, 300]

The sequence diagram shows how undo works for *add_expense* command:

.Sequence diagram of how *undo* command is applied on *add_expense* command
image::UndoAndRedo/UndoAddExpenseSequenceDiagram.png[align="center"]

Step 6. The user executes *redo* to add laksa back to THRIFT's transaction list.
The `RedoCommand` invokes the method `Model#getUndoneCommand()` to retrieve the `AddExpenseCommand` object.
Before returning the `AddExpenseCommand` back to the callee, `AddExpenseCommand` was pop from `RedoStack` and push into `UndoStack`.

image::UndoAndRedo/redo.PNG[align="center", 310, 300]

NOTE: All commands in `RedoStack` are purged when user executes another undoable command.

The sequence diagram shows how redo works for *add_expense* command:

.Sequence diagram of how *redo* command is applied on *add_expense* command
image::UndoAndRedo/RedoAddExpenseSequenceDiagram.png[align="center"]
